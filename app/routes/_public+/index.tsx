import type { V2_MetaFunction } from '@remix-run/node';

export const meta: V2_MetaFunction = () => {
  return [{ title: 'New Remix App' }, { name: 'description', content: 'Welcome to Remix!' }];
};

const images = [
  {
    src: 'https://image.shutterstock.com/image-vector/remix-brush-hand-drawn-typography-260nw-1788104798.jpg',
    alt: 'remix',
  },
  {
    src: 'https://tayming.com/upload/photos/2023/06/3LREefttf1UJQ5vqdngQ_14_baf14175732fef425bf7bea0fdc136c2_image.jpg',
    alt: 'tailwind',
  },
  {
    src: 'data:image/png;base64,',
    alt: 'Js',
  },
  {
    src: 'https://repository-images.githubusercontent.com/37153337/9d0a6780-394a-11eb-9fd1-6296a684b124',
    alt: 'react',
  },
  {
    src: 'https://cdn-clekk.nitrocdn.com/tkvYXMZryjYrSVhxKeFTeXElceKUYHeV/assets/images/optimized/rev-82dec77/wp-content/uploads/2020/12/node.js-logo-image.png',
    alt: 'node',
  },
];

export default function Index() {
  return (
    <div className="min-h-screen flex flex-col">
      <div className="flex-1 flex flex-col sm:flex-row">
        <main className="flex-1 bg-indigo-100 p-2">
          {images.map((image) => (
            <img src={image.src} alt={image.alt} className="max-w-xs p-2" />
          ))}
        </main>

        <nav className="order-first sm:w-48 bg-purple-200 p-2">
          Tutorials
          <ul className="ml-3">
            <li>
              <a href="https://remix.run/docs/en/main/tutorials/blog">Blog Tutorial (short)</a>{' '}
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/tutorials/jokes">App Tutorial (long)</a>
            </li>
          </ul>
          Pages
          <ul className="ml-3">
            <li>
              <a href="https://remix.run/docs/en/main/pages/v2">Preparing for v2</a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/philosophy">Philosophy</a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/technical-explanation">
                Technical Explanation
              </a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/contributing">Contributing</a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/api-development-strategy">
                API Development Strategy
              </a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/community">Community</a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/faq">FAQs</a>
            </li>
            <li>
              <a href="https://remix.run/docs/en/main/pages/gotchas">Gotchas</a>
            </li>
          </ul>
        </nav>
      </div>
    </div>
  );
}
